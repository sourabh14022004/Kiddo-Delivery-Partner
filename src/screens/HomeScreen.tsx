...
const OrderCard = memo(
  ({
    ...
    onPress: (orderId: string, order: ShopifyOrder) => void;
    onPick: (order: ShopifyOrder) => void;
  }) => {
    ...
    return (
      <TouchableOpacity
        style={styles.card}
        activeOpacity={0.85}
        onPress={() => onPress(order.id, order)}
      >
  ...
  (prevProps, nextProps) => {
    return (
      ...
      prevProps.pickingOrderId === nextProps.pickingOrderId &&
      prevProps.order === nextProps.order // Check reference equality
    );
  },
);

const HomeScreen: React.FC = () => {
  const { phoneNumber, pickerDetails, refreshHome } = useAppContext();
  ...
  const headerHeightAnim = useRef(
    new Animated.Value(HEADER_ROW_HEIGHT),
  ).current;
  ...
  const headerOpacity = scrollY.interpolate({
    inputRange: [
      ...
      HEADER_COLLAPSE_THRESHOLD + FADE_WINDOW,
    ],
    outputRange: [1, 1, 0],
    extrapolate: "clamp",
  });
  ...
  // Refresh when refreshHome changes (from AppContext notifications)
  useEffect(() => {
    if (refreshHome > 0) {
  ...
  const loadOrdersWithCache = async () => {
    ...
    // Only verify cache if not already showing data
    if (orders.length === 0) {
  ...
  const loadOrders = async (refresh = false, skipCache = false) => {
    try {
      if (refresh && !skipCache) {
        setRefreshing(true);
      } else if (!skipCache && orders.length === 0) {
        setLoading(true);
      }
      setError(null);
      ...
      // Improve error handling - only show if no data
      if (orders.length === 0) {
        setError(e.message || "Failed to load orders");
        // If we have cached data (long term), try fallback
  ...
  const handlePickOrder = useCallback(
    async (order: ShopifyOrder) => {
      ...
      if (!isProfileComplete(pickerDetails)) {
        Alert.alert("Profile Incomplete", getIncompleteProfileMessage(), [
          { text: "OK", style: "cancel" },
        ]);
        // Navigate to profile details if needed, via RootNavigator
        navigation.navigate("PickerDetails");
        return;
      }
      ...
      try {
        ...
        // Navigate to OrderDetails via RootNavigator
    ...
    [phoneNumber, pickerDetails, navigation],
  );

  const onViewOrderDetails = useCallback(
    (orderId: string, order: ShopifyOrder) => {
      // Pass the existing order data to preview screen for instant feedback
  ...
  const renderOrderItem = useCallback(
    ...
    [pickingOrderId, onViewOrderDetails, handlePickOrder],
  );
  ...
  return (
    <SafeAreaView style={styles.container} edges={[]}>
      ...
      <View style={styles.headerContainer} pointerEvents="box-none">
        {Platform.OS === "android" ? (
          <Animated.View
            style={[styles.headerCollapseSection, { height: headerHeightAnim }]}
          >
            <Animated.View
              style={[
                styles.header,
        ...
        {Platform.OS === "android" ? (
          <Animated.View
            style={[
              ...
              { paddingTop: searchPaddingTopAndroid },
            ]}
          >
          ...
          <View
            style={[
              styles.searchWrapper,
              headerCollapsed && { paddingTop: insets.top },
            ]}
          >
      ...
      {loading && isInitialLoad ? (
        ...
        <Animated.FlatList
          ...
          windowSize={5}
          removeClippedSubviews={Platform.OS === "android"} // Better for huge lists, sometimes risky on iOS for empty spaces
          refreshControl={
            <RefreshControl
...
/* ================= STYLES ================= */
// Copied existing styles to maintain UI consistency
